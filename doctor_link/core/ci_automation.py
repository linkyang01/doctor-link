...
SUCCESS_STATUSES = {"verified", "passed", "candidate_verified"}
TEST_SUCCESS_STATUSES = {"passed", "success", "verified"}
    ...
    def to_markdown(self) -> str:
        ...
        lines.extend(
            [
                ...
                "## Test Matrix Summary",
                "",
                _json_block(self.test_matrix_summary),
                "",
                "## Health Trend",
                "",
                _json_block(self.health_trend),
                "",
                "## Artifact Index",
                "",
            ]
        ...
        for artifact in self.artifact_index.get("artifacts", []):
            lines.append(
                f"- `{artifact.get('path')}` "
                f"({artifact.get('kind')}, {artifact.get('size_bytes')} bytes)"
            )
...
def build_ci_report(reports_dir: Path, output_dir: Path | None = None) -> CIAutomationReport:
    ...
    has_blocking_triage = any(item.get("severity") == "blocking" for item in triage)
    status = "passed" if regression_score >= 80 and not has_blocking_triage else "needs_attention"
...
def write_ci_report(reports_dir: Path, output_dir: Path | None = None) -> CIAutomationReport:
    ...
    report.artifact_index = _build_artifact_index(Path(report.reports_dir), out)
    (out / "ci-report.json").write_text(
        json.dumps(report.to_dict(), ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
    (out / "ci-report.md").write_text(report.to_markdown(), encoding="utf-8")
    (out / "github-step-summary.md").write_text(
        report.to_github_markdown(),
        encoding="utf-8",
    )
    (out / "ci-artifact-index.json").write_text(
        json.dumps(report.artifact_index, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
    (out / "project-health-trend.json").write_text(
        json.dumps(report.health_trend, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
    return report
...
def _package_dirs(reports_dir: Path) -> list[Path]:
    ...
    return sorted(
        ...
        if item.is_dir() and (item / "doctor-report.json").exists()
    )
...
def _build_failure_triage(package_dirs: list[Path]) -> list[dict[str, Any]]:
    ...
    for package_dir in package_dirs:
        ...
        comparison = _read_json(package_dir / "report-comparison.json", None)
        if comparison is None:
            comparison = _read_json(package_dir / "comparison-result.json", {})
        if not isinstance(verification, dict):
            verification = {}
        if not isinstance(pipeline, dict):
            pipeline = {}
        status = str(
            verification.get("status") or pipeline.get("verification_status") or "missing"
        )
        missing = _missing_evidence(verification)
        if status not in SUCCESS_STATUSES:
            items.append(
                {
                    "package": package_dir.name,
                    "severity": _verification_severity(status),
            ...
            items.append(
                {
                    ...
                    "severity": "warning",
                    "category": "pipeline",
                    "reason": "diagnosis pipeline success is false",
                }
            )
        comparison_text = _json_text(comparison)
            ...
            items.append(
                {
                    ...
                    "severity": "warning",
                    "category": "comparison",
                    "reason": "comparison contains regression or unresolved signal",
                }
            )
    return items
...
def _build_test_matrix_summary(package_dirs: list[Path]) -> dict[str, Any]:
    ...
    for package_dir in package_dirs:
        test_result_dir = package_dir / "evidence" / "test-results"
        for path in sorted(test_result_dir.glob("*.json")):
            payload = _read_json(path, {})
            if not isinstance(payload, dict):
                continue
            result_payload = payload.get("result")
            job_payload = payload.get("job")
            result = result_payload if isinstance(result_payload, dict) else payload
    ...
    total = len(jobs)
    failed = sum(1 for item in jobs if item.get("status") not in TEST_SUCCESS_STATUSES)
    passed = sum(1 for item in jobs if item.get("status") in TEST_SUCCESS_STATUSES)
...
def _build_health_trend(reports_dir: Path, package_dirs: list[Path]) -> dict[str, Any]:
    ...
    for package in health.packages:
        score = 100
        if package.get("verification_status") not in SUCCESS_STATUSES:
            score -= 35
    ...
    return {
        ...
        "direction": direction,
        "points": points,
        "source_package_count": len(package_dirs),
    }
...
def _verification_severity(status: str) -> str:
    return "blocking" if status in BLOCKING_VERIFICATION_STATUSES else "warning"


def _missing_evidence(verification: dict[str, Any]) -> list[Any]:
    missing = verification.get("missing_evidence")
    return missing if isinstance(missing, list) else []


def _json_text(value: Any) -> str:
    return json.dumps(value, ensure_ascii=False).lower() if isinstance(value, dict) else ""
