...
</style></head><body><a class="skip-link" href="#main-content">Skip to main content</a><header><h1>Doctor link Diagnostic Package Workbench</h1><p>{html.escape(subtitle)}</p></header><div class="layout"><nav aria-label="Package sections">{nav}</nav><main id="main-content">{main}</main></div>{_script()}</body></html>"""
