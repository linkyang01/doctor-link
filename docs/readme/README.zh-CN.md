...
版本 `v0.6.0` 新增结构化失败事实、精确源码定位、可逆根因实验、项目调用链、证据化修复建议、修复准入门和分层验收，支持 Python 与 Node.js 项目。
