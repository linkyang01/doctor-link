...
Doctor link 仍然坚持本地优先。版本 `0.1.2` 已在 2026-07-13 通过修复后的本地验证和 GitHub Actions 全矩阵验证：273 项测试、85.09% 分支覆盖率、Python 3.10–3.12、Ubuntu/macOS/Windows、安全检查、构建与隔离安装均通过。100 分认证运行见 [Actions 29221918410](https://github.com/linkyang01/doctor-link/actions/runs/29221918410)，按既定仓库发布就绪口径评分为 100/100。[GitHub Release `v0.1.2`](https://github.com/linkyang01/doctor-link/releases/tag/v0.1.2) 已从合并提交 `862281f` 正式发布；未执行 PyPI 发布。项目不创建托管平台、不引入外部账号体系、不做遥测、不提供 marketplace。

当前未发布加固进一步保证：失败的复现、必选测试和未完成验证会返回非零退出码；自动结果会回写诊断包；after 包继承 before 断言；只有关联到断言的 after 测试通过才可判定候选已验证；并发写入由事务锁和原子替换保护。完整规则见[自动诊断可靠性](../automated-diagnosis-reliability.md)。

当前未发布版本还增加了全能力验证实验室：针对源码和全新安装的 wheel，逐一执行全部 62 个公开 CLI 入口，以 70 次真实命令调用覆盖修复、并发、隐私、完整性、迁移、扩展、治理与归档等 8 组复杂场景。详见[全能力验证](../full-capability-validation.md)。
