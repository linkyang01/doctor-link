...
Doctor link 仍然坚持本地优先。源码版本 `0.1.2` 已在 2026-07-13 通过修复后的本地验证，但 `v0.1.2` 尚未发布为 GitHub Release。修复提交推送并通过 GitHub Actions 前，云端认证和正式发布状态均保持待验证。项目不创建托管平台、不引入外部账号体系、不做遥测、不提供 marketplace。
