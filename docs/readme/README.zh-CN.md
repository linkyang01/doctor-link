...
当前仓库源码是尚未发布的 `v0.3.0` 候选版本。JavaScript/TypeScript 自动解决和运行时韧性增强已通过本地 339 项测试、85.48% 分支覆盖率、安装后 wheel 的 72 次全能力调用与 10 组复杂场景，并完成一次保护验收输入不变的真实 Node.js 修复。[GitHub Actions 运行 29263187097](https://github.com/linkyang01/doctor-link/actions/runs/29263187097) 已在代码提交 `c9426e7` 上通过全部 10 个作业，覆盖 Python 3.10–3.14、安全、构建，以及 Ubuntu、macOS、Windows。合并、标签创建与发布仍未发生。详见[JavaScript/TypeScript 自动解决验证报告](../validation/javascript-typescript-solve-validation.md)。
