...
当前版本提供 `doctor-link solve`：针对干净 Git 仓库中的 Python 项目，先真实复现问题并生成修复预演；用户显式传入 `--allow-repair` 后，Doctor link 才创建独立分支、调用 Codex、最多迭代三轮，并独立重跑全部必选命令。测试、测试配置、复现与测试目录文件，以及检查命令直接引用的脚本都会先做哈希保护；Codex 说“已修复”或削弱测试都不算成功，只有在保护输入未变化的前提下重测全部通过才返回 `verified`。
...
Doctor link 仍然坚持本地优先。版本 `0.2.0` 已在 2026-07-13 通过修复后的本地验证和 GitHub Actions 全矩阵验证：319 项测试、85.39% 分支覆盖率、Python 3.10–3.12、Ubuntu/macOS/Windows、安全检查、构建与隔离安装均通过。[最终 PR CI 29256743976](https://github.com/linkyang01/doctor-link/actions/runs/29256743976) 的 8 个作业全部通过，按既定仓库发布就绪口径评分仍为 100/100。[GitHub Release `v0.2.0`](https://github.com/linkyang01/doctor-link/releases/tag/v0.2.0) 已从合并提交 `40a547c` 由 [Release 工作流 29256955705](https://github.com/linkyang01/doctor-link/actions/runs/29256955705) 正式发布；PyPI 步骤已关闭且未执行。项目不创建托管平台、不引入外部账号体系、不做遥测、不提供 marketplace。

版本 `0.2.0` 保留此前的证据与事务保障，并增加用户授权的自动修复、独立回归验收、验收输入哈希保护，以及验收条件被显式修改后的人工复核状态。完整规则见[Codex 自动解决](../automatic-solve.md)和[自动诊断可靠性](../automated-diagnosis-reliability.md)。

已发布的 `v0.2.0` 全能力验证实验室逐一执行全部 63 个公开 CLI 入口，以 71 次真实命令调用覆盖自动解决授权门禁、修复、并发、隐私、完整性、迁移、扩展、治理与归档等 9 组复杂场景。详见[全能力验证](../full-capability-validation.md)。
