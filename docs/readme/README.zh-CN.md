...
当前仓库源码是尚未发布的 `v0.3.0` 候选版本。JavaScript/TypeScript 自动解决扩展已通过本地 331 项测试、85.40% 分支覆盖率、安装后 wheel 的 72 次全能力调用与 10 组复杂场景，并完成一次保护验收输入不变的真实 Node.js 修复。云端验证、合并、标签创建与发布仍未发生，详见[JavaScript/TypeScript 自动解决验证报告](../validation/javascript-typescript-solve-validation.md)。
