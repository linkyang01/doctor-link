...
当前源码进一步提供 `doctor-link solve`：针对干净 Git 仓库中的 Python 项目，先真实复现问题并生成修复预演；用户显式传入 `--allow-repair` 后，Doctor link 才创建独立分支、调用 Codex、最多迭代三轮，并独立重跑全部必选命令。测试、测试配置、已配置目录和检查命令直接引用的脚本都会先做哈希保护；Codex 说“已修复”或削弱测试都不算成功，只有在保护输入未变化的前提下重测全部通过才返回 `verified`。
...
Doctor link 仍然坚持本地优先。版本 `0.1.3` 已在 2026-07-13 通过修复后的本地验证和 GitHub Actions 全矩阵验证：298 项测试、85.25% 分支覆盖率、Python 3.10–3.12、Ubuntu/macOS/Windows、安全检查、构建与隔离安装均通过。[最终 PR CI 29245883700](https://github.com/linkyang01/doctor-link/actions/runs/29245883700) 的 8 个作业全部通过，按既定仓库发布就绪口径评分仍为 100/100。[GitHub Release `v0.1.3`](https://github.com/linkyang01/doctor-link/releases/tag/v0.1.3) 已从合并提交 `fa779da` 由 [Release 工作流 29246045227](https://github.com/linkyang01/doctor-link/actions/runs/29246045227) 正式发布；PyPI 步骤已关闭且未执行。项目不创建托管平台、不引入外部账号体系、不做遥测、不提供 marketplace。

版本 `0.1.3` 进一步保证：失败的复现、必选测试和未完成验证会返回非零退出码；自动结果会回写诊断包；after 包继承 before 断言；只有关联到断言的 after 测试通过才可判定候选已验证；并发写入由事务锁和原子替换保护。完整规则见[自动诊断可靠性](../automated-diagnosis-reliability.md)。

当前源码的全能力验证实验室逐一执行全部 63 个公开 CLI 入口，以 71 次真实命令调用覆盖自动解决授权门禁、修复、并发、隐私、完整性、迁移、扩展、治理与归档等 9 组复杂场景。已发布的 `v0.1.3` 仍对应 62 个入口、70 次调用和 8 组场景的历史认证。详见[全能力验证](../full-capability-validation.md)。
...
P7 已实现证据加固、本地工作台加固、AI handoff runtime、CI 自动化、分发就绪检查、Adapter runtime、Plugin runtime、完整性与隐私门禁、本地知识索引、本地归档辅助能力。当前源码已在这些地基上增加 Python 自动解决纵切面。

## 边界说明

Doctor link 当前不提供托管 Web 平台、云同步、外部账号体系、遥测、marketplace、真实签名密钥或密钥管理、托管企业归档、托管诊断知识库或真实 RBAC。自动修复当前只支持 Python 项目，必须显式授权，保护原始验收输入，不自动提交、推送或发布；例外参数 `--allow-verification-changes` 只能得到 `review_required`，不能得到 `verified`。`--allow-repair` 会使用已有 Codex 登录调用服务，用户应先审查预演并移除敏感信息。PyPI 发布仍为可选项。
