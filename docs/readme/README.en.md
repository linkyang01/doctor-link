...
The current repository source is the unreleased `v0.3.0` candidate. Its JavaScript/TypeScript automatic-solve extension passed 329 local tests, 85.39% branch-aware coverage, 72 installed-wheel capability invocations across ten scenarios, and a live Node.js repair with unchanged protected acceptance inputs. Cloud validation, merge, tag creation, and publication are pending. See [JavaScript/TypeScript Automatic Solve Validation](../validation/javascript-typescript-solve-validation.md).
