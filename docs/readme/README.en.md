...
The current repository source is the unreleased `v0.3.0` candidate. Its JavaScript/TypeScript automatic-solve and runtime-resilience work passed 339 local tests, 85.48% branch-aware coverage, 72 installed-wheel capability invocations across ten scenarios, and a live Node.js repair with unchanged protected acceptance inputs. [GitHub Actions run 29261286975](https://github.com/linkyang01/doctor-link/actions/runs/29261286975) passed the earlier eight-job candidate matrix for code commit `9cec5d1`; the expanded Python 3.10–3.14 head matrix still requires cloud validation. Merge, tag creation, and publication are pending. See [JavaScript/TypeScript Automatic Solve Validation](../validation/javascript-typescript-solve-validation.md).
