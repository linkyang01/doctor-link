...
The retained benchmark corpus materializes 24 clean Git projects across Python and Node.js and twelve fault domains. The release run reproduced 24/24 failures with 100% expectation matching. Version 0.5.1 local preparation passed 365 tests at 85.06% branch-aware coverage, Ruff, Bandit with zero medium/high findings, dependency auditing with no known vulnerabilities, build/content/Twine gates, and the installed-wheel lab across 66 routes, 75 invocations, and twelve linked scenarios. PR `#154` run `29325710695` passed all ten Python 3.10–3.14, package, security, Ubuntu, macOS, and Windows jobs and merged as `ed7d240`. Release workflow `29325885785` published the immutable [`v0.5.1` GitHub Release](https://github.com/linkyang01/doctor-link/releases/tag/v0.5.1). Trusted Publishing workflow `29325997537` published identical wheel and source assets to [PyPI](https://pypi.org/project/doctor-link/0.5.1/) with attestations. A clean public-index `pipx install doctor-link==0.5.1` and installed CLI preflight passed.
...
P6 and P7 are complete as repository-side local-first implementation work. Version `0.5.1` is locally validated, cloud validated, merged, tagged, and published on GitHub and PyPI. This does not mean Doctor link has implemented a hosted platform, account system, marketplace, real signing, hosted archive, or hosted knowledge base.
...
- [x] GitHub Release `v0.5.1` published (2026-07-14)
...
- [x] PyPI `doctor-link==0.5.1` published (2026-07-14)
